...
def dedup_date(spark_session):
    ...
    StructField("dim_team_abbreviation", StringType(), True),
    ...
    StructField("m_seconds_played", IntegerType(), True),
    StructField("m_field_goals_made", DoubleType(), True),
    StructField("m_field_goals_attempted", DoubleType(), True),
    StructField("m_3_pointers_made", DoubleType(), True),
    StructField("m_3_pointers_attempted", DoubleType(), True),
    StructField("m_free_throws_made", DoubleType(), True),
    StructField("m_free_throws_attempted", DoubleType(), True),
    StructField("m_offensive_rebounds", DoubleType(), True),
    StructField("m_defensive_rebounds", DoubleType(), True),
    StructField("m_rebounds", DoubleType(), True),
    StructField("m_assists", DoubleType(), True),
    StructField("m_steals", DoubleType(), True),
    StructField("m_blocks", DoubleType(), True),
    StructField("m_turnovers", DoubleType(), True),
    StructField("m_personal_fouls", DoubleType(), True),
    StructField("m_points", DoubleType(), True),
    StructField("m_plus_minus", DoubleType(), True),
    StructField("dim_game_date", DateType(), True),
    StructField("dim_season", IntegerType(), True),
    StructField("dim_team_did_win", BooleanType(), True)
    ])

    actual_fct_nba_game_details = [
    fct_nba_game_details(
        game_id=1, 
        team_id=100, 
        player_id=1000, 
        dim_team_abbreviation="LAL", 
        dim_player_name="LeBron James", 
        dim_start_position="SF", 
        dim_did_not_dress=False, 
        dim_not_with_team=False, 
        m_seconds_played=2400, 
        m_field_goals_made=10, 
        m_field_goals_attempted=20, 
        m_3_pointers_made=2, 
        m_3_pointers_attempted=5, 
        m_free_throws_made=5, 
        m_free_throws_attempted=6, 
        m_offensive_rebounds=1, 
        m_defensive_rebounds=7, 
        m_rebounds=8, 
        m_assists=9, 
        m_steals=2, 
        m_blocks=1, 
    ...
    fct_nba_game_details(
        ...
        team_id=101, 
        player_id=1001, 
        dim_team_abbreviation="GSW", 
        dim_player_name="Stephen Curry", 
        dim_start_position="PG", 
        dim_did_not_dress=False, 
        dim_not_with_team=False, 
        m_seconds_played=2400, 
        m_field_goals_made=12, 
        m_field_goals_attempted=22, 
        m_3_pointers_made=5, 
        m_3_pointers_attempted=10, 
        m_free_throws_made=4, 
        m_free_throws_attempted=4, 
        m_offensive_rebounds=0, 
        m_defensive_rebounds=5, 
        m_rebounds=5, 
        m_assists=7, 
        m_steals=1, 
        m_blocks=0, 
    ...
    fct_nba_game_details(
        ...
        m_points=27, 
        m_plus_minus=15, 
        dim_game_date=datetime.strptime("2023-01-01", "%Y-%m-%d"), 
        dim_season=2023, 
        dim_team_did_win=True
    ),
    fct_nba_game_details(
...
def test_inc_host_data(spark_session):
    ...
    actual_web_event_data = [
            ...
            WebEvents(
            host = "www.example.com",
            event_time = "2023-01-01 00:05:39.907"
           ),
            WebEvents(
                host = "admin.zachwilson.io.tech",
                event_time = "2023-01-01 00:01:39.907"
            ),
            WebEvents(
                host = "admin.zachwilson.testing.tech",
