...
class SparkJobTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.spark = SparkSession.builder \
            .master("local[1]") \
            .appName("unit_test_for_spark_jobs") \
            .getOrCreate()
    ...
    def test_job_1(self):
        """Test case for job_1"""
        # This will help to define the schema and test the data
        schema = StructType([
        ...
        data = [
            ("1", "Actor One", [("Film A", 50, 9.5, "F1")], "high", True, 2024),
            ("2", "Actor Two", [("Film B", 300, 8.2, "F2")], "low", False, 2024)
        ]
    ...
    def test_job_2(self):
        ...
        data = [
            ("Actor One", [("Film A", 50, 9.5, "F1"), ("Film C", 450, 9.0, "F3")]),
            ("Actor Two", [("Film B", 300, 8.2, "F2")])
        ]
        df = self.spark.createDataFrame(data, schema)
        df.createOrReplaceTempView("test_actors")
