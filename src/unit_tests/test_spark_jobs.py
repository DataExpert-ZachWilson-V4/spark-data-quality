...
NbaGameDetails = namedtuple(
    "NbaGameDetails",
    "game_id team_id player_id team_abbreviation team_city nickname player_name",
)
NbaGameDetailsDeduped = namedtuple(
    "NbaGameDetailsDeduped",
    "game_id team_id player_id team_abbreviation team_city nickname player_name",
)
...
def test_job_1(spark_session: SparkSession):
    input_table_name: str = "nba_game_details"
    input_data = [
        NbaGameDetails(
            20701032, 1610612758, 201150, "SAC", "Sacramento", None, "Spencer Hawes"
        ),
        NbaGameDetails(
            20701032, 1610612758, 201150, "SAC", "Sacramento", None, "Spencer Hawes"
        ),
        ...
        NbaGameDetails(
            20400555, 1610612753, 278, "ORL", "Orlando", None, "Stacey Augmon"
        ),
        NbaGameDetails(
            20400555, 1610612753, 278, "ORL", "Orlando", None, "Stacey Augmon"
        ),
...
def test_job_2(spark_session: SparkSession):
    ...
    daily_data = [
        WebEvent(
            ...
            "/",
            datetime.strptime(
                "2023-01-02 07:00:34.627 UTC", "%Y-%m-%d %H:%M:%S.%f UTC"
            ),
        ),
        WebEvent(
            -2077270748,
            ...
            "/",
            datetime.strptime(
                "2023-01-02 07:15:32.122 UTC", "%Y-%m-%d %H:%M:%S.%f UTC"
            ),
        ),
        WebEvent(
            1917610522,
            ...
            "/robots.txt",
            datetime.strptime(
                "2023-01-02 07:05:00.648 UTC", "%Y-%m-%d %H:%M:%S.%f UTC"
            ),
        ),
    ]
    ...
    expected_data = [
        ...
        HostCumulated(
            "admin.zachwilson.tech",
            [None, datetime.strptime("2023-01-01", "%Y-%m-%d")],
            datetime.strptime("2023-01-02", "%Y-%m-%d"),
        ),
    ]
