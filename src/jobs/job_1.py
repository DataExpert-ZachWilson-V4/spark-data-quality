...
def query_1(output_table_name: str) -> str:
    ...
    WITH last_year AS 
        (
            SELECT * FROM {output_table_name} WHERE current_year = 1924
        ),
    this_year AS 
        (
            SELECT
    ...
    CASE
        ...
        CASE
            WHEN ty.rating > 8.0 THEN 'star'
            WHEN ty.rating > 7.0 AND ty.rating <= 8.0 THEN 'good'
            WHEN ty.rating > 6.0 AND ty.rating <= 7.0 THEN 'average'
            ELSE 'bad'
        END
