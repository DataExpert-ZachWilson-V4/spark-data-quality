...
def query_1(output_table_name: str) -> str:
    """
    Constructs a SQL query to perform an upsert operation on the specified table. 
    This query merges existing data with new data,
    updates existing records based on the new data, and inserts new records 
    when no existing record matches.

    Args:
    output_table_name (str): The name of the table to perform the merge operation on.

    Returns:
    str: A SQL query f string configured for performing an upsert (merge) operation.
    """
    query = f"""
...
def job_1(spark_session: SparkSession, output_table_name: str) -> Optional[DataFrame]:
    ...
    This job function is pivotal in ensuring that the table data is current and 
    accurately reflects the latest available data.
...
def main():
    """
    Main executable function to trigger the merge job using a Spark session.
    This function setups the Spark session, calls the job function, and handles the output.
    """
    output_table_name: str = "jimmybrock65656.actors"  # Designate the target table name
