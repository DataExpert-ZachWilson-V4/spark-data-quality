...
def job_1(spark_session: SparkSession, source: str, output_table_name: str) -> Optional[DataFrame]:
    # this is the base source of the query (lagged)
    input_df = spark_session.table(source)
    input_df.createOrReplaceTempView(source)

    # this is the table in which the results will be saved
    output_df = spark_session.table(output_table_name)
    output_df.createOrReplaceTempView(output_table_name)
    return spark_session.sql(query_1(source))
...
def main():
    ...
    output_df = job_1(spark_session, source)
